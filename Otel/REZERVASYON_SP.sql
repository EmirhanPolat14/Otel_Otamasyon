...
BEGIN
	...
	CREATE TABLE #TEL(ID INT IDENTITY(1,1), DEGER CHAR(15))
	...
	INSERT INTO #DOGUM(DEGER) SELECT CONVERT(DATE,VALUE,104) FROM string_split(@DOGUM_TARIH,',')
	...
	BEGIN	
		...
		INSERT INTO Musteri(Tc_No, Isim, Soyisim,  Dogum_Tarihi, Cinsiyet, Telefon, E_mail,  Oda_No,Aktif)
		SELECT  
		...
		BEGIN
			...
			SET @TUTAR = (SELECT dbo.UCRET(0,@ODA_NO, DATEDIFF(DAY,@BASLANGICTARIHI,@BITISTARIHI), @EK_YATAK))
