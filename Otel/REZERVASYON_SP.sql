...
--  @ODA_DURUM ile odanýn müsaitlik durumu ve en sonunda da ayný tarihler arasýnda odaya baþka bir rezervasyon olup olmadýðýnýn kontrolü yapýlýr.
IF ( ( @ODA_KISI = 0 AND @ODA_DURUM = 1) AND 
...
BEGIN
	...
	BEGIN	
		...
		INSERT INTO Musteri(Isim,Soyisim,Tc_No,Dogum_Tarihi,Cinsiyet,Telefon,E_mail,Oda_No,Ek_Yatak,Kisi_sayisi)
		SELECT  
			...
			@TELEFON,@MAIL,@ODA_NO,@EK_YATAK,@KISI
	...
	PRINT CONCAT_WS(' ',@BASLANGICTARIHI, 'ile', @BITISTARIHI, 'tarihleri arasýndaki ',@REZID,'numaralý rezervasyonunuz baþarýyla oluþturuldu.')
